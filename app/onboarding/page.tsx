...
export default function OnboardingPage() {
  ...
  const [hasT1D, setHasT1D] = useState<boolean | null>(null)
  const condition = hasT1D ? 'Type 1 Diabetes' : (hasT1D === false ? 'Other' : '')
  ...
  const [manufacturerContactType, setManufacturerContactType] = useState<'routine' | 'reactive' | 'both' | null>(null)
  ...
  const handleName = () => {
    ...
    const q = isMyself ? 'do you have Type 1 Diabetes?' : `does ${first} have Type 1 Diabetes?`
    aiSay(`Nice to meet you, ${first}! Quick confirmation — ${q}`, 2)
  }

  const handleT1D = (val: boolean) => {
    setHasT1D(val)
    userSay(val ? 'Yes' : 'No')
  ...
  const handleDeviceYesNo = (val: boolean) => {
    ...
    if (val) {
      ...
      aiSay(`Got it. Should Amphelo contact ${their} medication manufacturers on ${their} behalf — and if so, how?`, 6)
    }
  }
  ...
  const handleDevices = () => {
    ...
    aiSay(`Got it. Should Amphelo contact ${their} device or medication manufacturers on ${their} behalf — and if so, how?`, 6)
  }

  const handleManufacturerType = (type: 'routine' | 'reactive' | 'both' | 'none') => {
    const labels: Record<string, string> = {
      routine: 'Scheduled outreach (routine reorders)',
      reactive: 'On-demand support (device issues & replacements)',
      both: 'Both',
      none: 'Not needed',
    }
    setHasManufacturers(type !== 'none')
    if (type !== 'none') setManufacturerContactType(type as any)
    userSay(labels[type])
    if (type !== 'none') {
      aiSay(`Got it. Add each manufacturer's name and customer service number.`, 7)
    } else {
  ...
  const handleFinish = () => {
    ...
    // Specialist check-in workflows
    const specialistWorkflows: Workflow[] = prescriberEntries
      .filter(p => p.name.trim() && p.visitFrequency.trim())
      .map(p => {
        const freqDays = parseFrequencyToDays(p.visitFrequency)
        const nextDate = p.nextVisitDate
          ? new Date(p.nextVisitDate).toISOString()
          : new Date(Date.now() + freqDays * 24 * 60 * 60 * 1000).toISOString()
        const phone = p.phone || ''
        return {
          id: makeId(), type: 'custom' as const,
          title: `Check in with ${p.name}`,
          description: `Call ${[p.practice, p.name].filter(Boolean).join(' — ')} to confirm upcoming appointment (${p.visitFrequency})`,
          pharmacyName: p.practice || p.name, pharmacyPhone: phone,
          status: 'needs_approval' as const,
    ...
    setState({
      ...
      workflows: [...medWorkflows, ...deviceWorkflows, ...specialistWorkflows],
      messages: [],
      onboardingComplete: true,
      ...({
        condition, submittingFor, hasT1D,
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 max-w-2xl w-full mx-auto">
        ...
        {/* Step 2: T1D confirmation */}
        {!typing && step === 2 && (
          <div className="flex gap-2 mt-1 mb-4 pl-9">
            <button onClick={() => handleT1D(true)} className="px-4 py-2 bg-blue-50 border border-blue-200 text-blue-700 rounded-xl text-sm font-medium hover:bg-blue-100 transition">Yes</button>
            <button onClick={() => handleT1D(false)} className="px-4 py-2 bg-slate-100 border border-slate-200 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-200 transition">No</button>
          </div>
        )}
        ...
        {!typing && step === 5 && (
          <FormCard>
            {deviceEntries.map((d, i) => {
              const typePh  = i === 0 ? 'CGM' : 'Insulin Pump'
              const brandPh = i === 0 ? 'Dexcom' : 'Omnipod'
              const modelPh = i === 0 ? 'G7' : '5'
              return (
              <div key={d.id} className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <Field label="Device type" placeholder={typePh} value={d.devType}
                    onChange={e => setDeviceEntries(es => es.map(x => x.id === d.id ? { ...x, devType: e.target.value } : x))} />
                  <Field label="Brand" placeholder={brandPh} value={d.brand}
                    onChange={e => setDeviceEntries(es => es.map(x => x.id === d.id ? { ...x, brand: e.target.value } : x))} />
                  <Field label="Model" placeholder={modelPh} value={d.model}
        ...
        {/* Step 6: Manufacturer contact type */}
        {!typing && step === 6 && (
          <div className="pl-9 mt-1 mb-4 space-y-2">
            <button onClick={() => handleManufacturerType('routine')} className="w-full text-left px-4 py-3 bg-blue-50 border border-blue-200 text-blue-700 rounded-xl text-sm font-medium hover:bg-blue-100 transition">
              <span className="font-semibold">Scheduled outreach</span>
              <span className="block text-xs text-blue-500 mt-0.5">For routine reorders and refills directly with the manufacturer</span>
            </button>
            <button onClick={() => handleManufacturerType('reactive')} className="w-full text-left px-4 py-3 bg-slate-50 border border-slate-200 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-100 transition">
              <span className="font-semibold">On-demand support</span>
              <span className="block text-xs text-slate-400 mt-0.5">When issues come up — device failures, peel-offs, or replacement supplies</span>
            </button>
            <button onClick={() => handleManufacturerType('both')} className="w-full text-left px-4 py-3 bg-slate-50 border border-slate-200 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-100 transition">
              <span className="font-semibold">Both</span>
              <span className="block text-xs text-slate-400 mt-0.5">Routine reorders and reactive support as needed</span>
            </button>
            <button onClick={() => handleManufacturerType('none')} className="px-4 py-2 text-slate-400 text-xs hover:text-slate-600 transition">Not needed →</button>
          </div>
        )}
